...
import { MapPin, Clock, Phone, Globe, VolumeIcon as VolumeUp, User, MessageCircle } from "lucide-react"
import {
  type Location,
  type Phrase,
  type Vocabulary,
...
export default function LocationDetail({ location }: LocationDetailProps) {
  ...
  useEffect(() => {
    const fetchRelatedData = async () => {
      const phrases = await getRelatedPhrases(location.id)
      const vocabulary = await getRelatedVocabulary(location.id)
      const nearby = await getNearbyLocations(location.id)

      setRelatedPhrases(phrases)
      setRelatedVocabulary(vocabulary)
